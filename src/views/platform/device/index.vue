    ...
    <el-container>
        ...
        <el-main class="nopadding">
            <sc-table ref="tableRef" :apiObj="list.apiObj" row-key="id" @selection-change="selectionChange" stripe>
                ...
                <el-table-column label="设备标识" prop="key" width="180"></el-table-column>
                <el-table-column label="备注信息" prop="remark"></el-table-column>
                <el-table-column label="状态" prop="status" width="60">
                    <template #default="scope">
                        <sc-status-indicator v-if="scope.row.is_online" type="success"></sc-status-indicator>
                        <sc-status-indicator v-if="!scope.row.is_online" pulse type="danger"></sc-status-indicator>
                    </template>
                </el-table-column>
...
import deviceApi from "@/api/platform/device"
...
const tableDel = async (row, index) => {
    ...
    const res = await deviceApi.delete.post(reqData);
    if (res.code === 0) {
...
const batch_del = async () => {
    ...
    const res = await deviceApi.delete.post({id: ids});
    if (res.code === 0) {
        tableRef.value.refresh()
